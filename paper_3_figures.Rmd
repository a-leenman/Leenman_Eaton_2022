...
Initial plots for third paper with constant feed, varied sediment supply. Trying to decide if I should compare runs 1-6-7 or runs 1-7-8-9...
...
mtext(text = "Feed rate (g/s)", side = 2, outer = T, line = 0)
dev.off()
```
...
#   }
# }
# title(main = "Median slope with varying feed amplitude",
#       outer = T,
#       line = -2)
# mtext(side = 1, text = "Time in cycle (mins)",
#       outer = T, line = -2)
# mtext(text = "Median fan slope", side = 2, outer = T, line = -2)
...
for(i in 1:length(exps)){
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, 0.055, "H-F", cex = 2)
    text(cycle.len/2 + cycle.len/4, 0.055, "0-F", cex = 2)
  }
...
for(i in 1:length(exps)){
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, 0.06, "H-F", cex = 2)
    text(cycle.len/2 + cycle.len/4, 0.06, "0-F", cex = 2)
  }
...
for(i in c("high", "low")){
  ...
  for(j in 1:4){
    ...
    plot(raster_sum, 
         ...
         legend = F)
    legend("topright", legend = paste0("Run ", run, " ", i, " feed"),
           cex = 2)
...
```{r cross-section data wrangling 0.5 m,eval=F}
# empty list
mylist <- list()
...
for(i in c(1, 3:5)){
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, 0.12, "H-F", cex = 2)
    text(cycle.len/2 + cycle.len/4, 0.12, "0-F", cex = 2)
  }
...
for(i in c(1, 3:5)){
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, 0.05, "H-F", cex = 2)
    text(cycle.len/2 + cycle.len/4, 0.05, "0-F", cex = 2)
  }
...
for(i in c(1, 3:5)){
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, -3, "H-F", cex = 2)
    text(cycle.len/2 + cycle.len/4, -3, "0-F", cex = 2)
  }
...
plot(raster_sum,
     ...
     legend.args=list(
       text='Mean deposition/erosion depth (mm)', side=4, font=1, line=3, cex=1.8),
