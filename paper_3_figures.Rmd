...
legend(10, 16,
       bty = "n",
       ncol = 4,
       legend = c("CON", "OSC10", "OSC20", "OSC40"),
       col = colz[c(1, 3:5)],
       lwd = 2)
...
boxplot(slope_50pc*-1 ~ run, data = slope50_1_7_8_9,
        ...
        xlab = "Run",
        ylab = expression("Median fan slope (m m"^-1*")"),
...
for(i in 1:length(exps)){
  ...
  nrep <- length(unique(dat$exprep))
  for(eachrep in 1:nrep){
    dat_i <- dat %>%
      dplyr::filter(exprep == eachrep)
    # add lines for each cycle
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c(abs(dat_i$slope_50pc)[dat_i$cycle == (j-1) & 
                                      dat_i$time_in_cycle == 10],
              abs(dat_i$slope_50pc)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i]))
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(signif (mean(abs(dat$slope_50pc)), 2), 
                    signif (sd(abs(dat$slope_50pc)),1)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz[i],
...
mtext(side = 1, text = "Time in cycle (mins)",
      outer = T, line = -0.5)
mtext(text = expression("Median fan slope (m m"^-1*")"), 
      side = 2, outer = T, line = -0.3)
dev.off()
...
for(i in 1:length(exps)){
  ...
  rep1sup <- dat %>%
    dplyr::filter(exprep == 1)
  start.row <- min(which(rep1sup$cycle==2)) # index of first row of first complete cycle (only for rep 1, as only needs calc once)
  end.row <- max(which(rep1sup$cycle==2)) # index of last row of first complete cycle
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, 0.0555, "H-S", cex = 2)
    text(cycle.len/2 + cycle.len/4, 0.0555, "0-S", cex = 2)
  }
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0, # paste a 0 on the front, so t10 (or t_final) value in prev cycle 
              ...
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]), 
            c(abs(dat_i$slope_50pc)[dat_i$cycle == (j-1) & dat_i$time_in_cycle == cycle.len],
              abs(dat_i$slope_50pc)[dat_i$cycle == j]),
            #lwd = 0.5,
...
Comparing axial (45 degree) slope to mid-flank (e.g. 22.5 degree) slope:
```{r axial_Vs_flank_wrangling, echo = F}
# import axial, 23 and 67 degrees slope for all fans, all timesteps
# empty list
mylist <- list()
poi <- c('p23', 'p45', 'p67') # profiles of interest

# vector of runs/reps/drives
repz <- c(3, 1, 1, 1, 1)
runnames <- c("CON", "OSC10LA",   "OSC10", "OSC20", "OSC40")
...
for(i in 1:length(runs)){
  ...
  for(repnum in 1:nreps){
    repe <- repnum
    t_list <- list.files(
      paste0(drive, "://Experiments/Processing/Run", run, "/Run", run, "_rep", repe,
             "/_1min_intervals/t_series/Recalculating_slope/profile_summaries"))
    repdat <- data.frame()
    # loop through each timestep to grab relevant slopes
    for(tstep in 1:length(t_list)){
      # time wrangling
      tvec <- substr(t_list[tstep], 10, 13)
      hr <- substr(tvec, 1, 2)
      mn <- substr(tvec, 3, 4)
      t_sec <- period_to_seconds(hm(paste0(hr, ':', mn))) # number of seconds since 0
      # import relevant slope data, extracted from DEMs prior
      stepdat <- read.csv(
        paste0(drive, "://Experiments/Processing/Run", run, "/Run", run, "_rep", repe,
               "/_1min_intervals/t_series/Recalculating_slope/profile_summaries/",
               t_list[tstep])) %>% 
        dplyr::filter(prof_names %in% poi) %>%
        dplyr::select(prof_names, slope) %>%
        mutate(slope = abs(slope), t_sec = t_sec) %>%
        pivot_wider(names_from = prof_names, values_from = slope) %>%
        cbind(data.frame(tvec = tvec), .) # export to df row
      
      repdat <- rbind(repdat, stepdat) # bind the rows
    }
    # assign timestamps / data identifiers
    repdat <- repdat %>% 
      mutate(t_min = t_sec/60, t_hr = t_min / 60) %>% 
      mutate(exprep = repe, run = run) %>%
      mutate(flank_av_slope = (p23 + p67)/2) %>% # average flank slope
      mutate(ax_minus_flank = p45 - flank_av_slope) 
    ...
    df_all <- rbind(df_all, repdat) # bind all repeats into one df
  }
  df <- df_all
...
# plot cycles through time, as for other variables
png(paste0(outfiles, "/flank_minus_axial_slope_vs_timeincycle.png"),
     width = 960, height = 480)
par(mai = c(0.5,0.4,0.1,0.1),
    oma = c(3, 2.5, 0, 0))
layout(matrix(c(1,2,3,4), 1, 4, byrow = TRUE),
...
for(i in c(1, 3:5)){
  ...
  # blank axes
  plot(0:max(unique(dat$time_in_cycle)),
       abs(dat$ax_minus_flank)[(start.row-1):end.row],
       type = "n",
       xlab = "",
       ylab = "",
       ylim = c(-0.01, 0.01),
  ...
  if(unique(dat$run)>1){
    ...
    abline(v = cycle.len/2 + 0.5, lty = 2,
           col = "gray50")
    # labels
    text(cycle.len/4, -0.01, "H-S", cex = 2)
    text(cycle.len/2 + cycle.len/4, -0.01, "0-S", cex = 2)
  }
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              dat_i$time_in_cycle[dat_i$cycle == j]),
            c(dat_i$ax_minus_flank[dat_i$cycle == (j-1) &
                                     dat_i$time_in_cycle == cycle.len],
              dat_i$ax_minus_flank[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i]))
    }
  }
  ...
  # get mean for group; add to plot
  df2 = group_by(dat, time_in_cycle)
  df2m = summarize(df2, mean_of_means = mean(ax_minus_flank))
  lines(0:cycle.len,
        c(pull(df2m[cycle.len, 2]), pull(df2m[,2])),
        col = darken(colz[i], amount = 0.4, space = "combined"),
        lwd = 2)
  ...
  legend("topright",
         box.col = 'white',
         legend = c(format(round(mean(dat$ax_minus_flank), 3), nsmall = 1), 
                    signif (sd(dat$ax_minus_flank),1)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz[i],
...
mtext(side = 1, text = "Time in cycle (mins)",
      outer = T, line = 0, cex = 1.5)
mtext(text = expression("Slope differential (m m"^-1*")"), 
      side = 2, outer = T, line = 0, cex = 1.5)
...
for(i in 1:length(exps)){
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c((dat_i$new_area_normby_t2_fan * 100)[dat_i$cycle == (j-1) & dat_i$time_in_cycle == cycle.len],
              (dat_i$new_area_normby_t2_fan * 100)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz2[i], alphas[i]))
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(format(signif(mean(dat$new_area_normby_t2_fan * 100), 2), nsmall = 1), 
                    signif(sd(dat$new_area_normby_t2_fan * 100),2)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz2[i],
...
for (j in 1:length(runz)){
  ...
  print(paste0("Run ", run, " rep ", repe, " is done!"))
}
...
cleandata <- df %>%
  ...
  rename(time_step_hhmm_format = tvec, time_in_sec = t_sec,
         time_in_min = t_min, time_in_hrs = t_hr,
         num_channels = dividedness, min_in_cycle = time_in_cycle) 
write.csv(cleandata, 
          file = paste0(drive, "://Experiments/Processing/Run", run, "/Run", run, 
                        "_rep", repe,
                        "/_1min_intervals/t_series/cross-section-analyses/Run", 
                        run, "_all_repeats_n_chan_0.5m.csv"), row.names = F)
}
...
for(i in c(1, 3:5)){
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c(abs(dat_i$dividedness)[dat_i$cycle == (j-1) &
                                     dat_i$time_in_cycle == cycle.len],
              abs(dat_i$dividedness)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i]))
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(signif(mean(dat$dividedness), 2), 
                    format(signif(sd(dat$dividedness),2), nsmall = 1)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz[i],
...
for(i in 1:length(runs)){
  ...
  write.csv(cleandata, 
            file = paste0(drive, "://Experiments/Processing/Run", run, "/Run", run, 
                          "_rep", repe,
                          "/_1min_intervals/t_series/cross-section-analyses/Run", 
                          run, "_all_repeats_n_chan_1m.csv"), row.names = F)
  # active sector
  cleandata <- df %>%
    dplyr::select(tvec, ch_sector, t_sec,t_min,t_hr, 
                  cycle, time_in_cycle, run, exprep) %>%
    rename(time_step_hhmm_format = tvec, time_in_sec = t_sec,
           time_in_min = t_min, time_in_hrs = t_hr,
           active_sector_width_m = ch_sector,
           min_in_cycle = time_in_cycle) 
  write.csv(cleandata, 
            file = paste0(drive, "://Experiments/Processing/Run", run, "/Run", run, 
                          "_rep", repe,
                          "/_1min_intervals/t_series/cross-section-analyses/Run", 
...
for(i in c(1, 3:5)){
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c(abs(dat_i$ch_sector)[dat_i$cycle == (j-1) &
                                     dat_i$time_in_cycle == cycle.len],
              abs(dat_i$ch_sector)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i]))
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(format(signif(mean(dat$ch_sector), 2), nsmall = 2), 
                    format(signif(sd(dat$ch_sector),2), nsmall = 2)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz[i],
...
for(i in 1:length(runs)){
  ...
  cleandata <- df %>%
    ...
    rename(time_step_hhmm_format = time, time_in_sec = t_sec,
           time_in_min = t_min, time_in_hrs = t_hr, min_in_cycle = time_in_cycle,
           wet.area.m = wet.area, fan.area.m = fan.area) 
  write.csv(cleandata, 
            file = paste0(drive, "://Experiments/Processing/Run", run, "/Run", run, 
                          "_rep", repe,
...
for(i in c(1, 3:5)){
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c(abs(dat_i$wet.fraction)[dat_i$cycle == (j-1) &
                                     dat_i$time_in_cycle == cycle.len],
              abs(dat_i$wet.fraction)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i]))
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(format(signif(mean(dat$wet.fraction), 2), nsmall = 2), 
                    signif(sd(dat$wet.fraction),1)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz[i],
...
for(i in c(1, 3:5)){
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c(abs(dat_i$agg)[dat_i$cycle == (j-1) & dat_i$time_in_cycle == cycle.len],
              abs(dat_i$agg)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(rdbu[length(rdbu)], alphas[i])) # blue for agg
    }
  }
  ...
  for(eachrep in 1:nrep){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
              abs(dat_i$time_in_cycle)[dat_i$cycle == j]),
            c(abs(dat_i$deg)[dat_i$cycle == (j-1) & dat_i$time_in_cycle == cycle.len],
              abs(dat_i$deg)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(rdbu[1], alphas[i])) # red for deg
    }
  }
...
png(paste0(outfiles, "/aggdeg_vs_timeincycle_sep_panels.png"),
...
for(i in c(1, 3:5)){
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
            ...
            c(abs(dat_i$agg)[dat_i$cycle == (j-1) & dat_i$time_in_cycle == cycle.len],
              abs(dat_i$agg)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i])) # blue for agg
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(signif(mean(dat$agg, na.rm = T), 3), 
                    signif(sd(dat$agg, na.rm = T),2)),
         title.font = 1,
         title.col = 'black',
         text.font = c(2, 3),
         text.col = colz[i],
    ...
    for (j in 2:(max(dat_i$cycle) - 1)){ # prune out incomplete cycles
      lines(c(0,
            ...
            c(abs(dat_i$deg)[dat_i$cycle == (j-1) & dat_i$time_in_cycle == cycle.len],
              abs(dat_i$deg)[dat_i$cycle == j]),
            #lwd = 0.5,
            col = alpha(colz[i], alphas[i])) # red for deg
    }
  }
  ...
  legend("topright",
         box.col = 'white',
         legend = c(signif(mean(abs(dat$deg), na.rm = T), 2), 
                    signif(sd(dat$deg, na.rm = T),2)),
         text.font = c(2, 3),
         text.col = colz[i],
         cex = 2,
         bg = 'white',
...
mtext(side = 1, text = "Time in cycle (mins)",
      outer = T, line = 0,
      cex = 1.5)
mtext(text = expression("Erosion volume (cm"^3*")                                                           Deposition volume (cm"^3*")"), 
      side = 2, outer = T, line = -0.2,
      cex = 1.5)
...
for(i in c("high", "low")){
  ...
  for(j in 1:length(runs)){
    ...
    # title
    if(run == 1){
      title(main = "              CON, half data",
          line = -1,
          cex.main = 1)
    } else {
      title(main = paste0("              ", runnames[j], ", ", i, " supply"),
          line = -1,
          cex.main = 1)
    }
